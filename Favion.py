...
try:
    ...
    st.error("API Key belum diisi. Manage app → Settings → Secrets")
    st.stop()
...
.stButton>button[data-testid="scroll-btn"]:hover{{background:{T['user_bg']}!important}}
.stButton>button[data-testid="scroll-btn"] p{{font-size:18px!important;margin:0!important;color:{T['text']}!important}}
...
.tts-icon{{background:{T['badge_bg']};border:1px solid {T['border']};border-radius:50%;width:32px;height:32px;margin-top:8px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:1rem;color:{T['badge_text']}}}
.tts-icon:hover{{background:{T['user_bg']};color:{T['text']}}}
[data-testid="stAudioInput"]{{margin-bottom:10px!important}}
...
.typing-indicator span{{width:8px;height:8px;background-color:{T['primary']};border-radius:50%;display:inline-block;animation:bounce 1.4s infinite ease-in-out both}}
...
def transcribe_audio(audio_bytes):
    try:
        ...
        if len(t) < 3 or t.lower() in ["dan abroh", "terima kasih", "you", ""]:
            toast("Suara gak kedeteksi jelas", "⚠️")
            return ""
        return t
    except Exception as e:
        toast(f"STT Error: {str(e)[:30]}", "❌")
        return ""
...
def generate_gambar(p):
    ...
    st.session_state.last_generated_prompt = p
...
def remix_gambar_hasil_generate(pr):
    if not st.session_state.last_generated_prompt:
        return None, "Buat gambar dulu baru bisa di-remix"
    toast("Maaf jika hasilnya kurang memuaskan 🙏", "✨")
    fp = f"{st.session_state.last_generated_prompt}, {pr}"
    st.session_state.last_generated_prompt = fp
...
def kirim_ke_ai(prompt, image=None):
    ...
    if is_sensitif:
        return [("text", f"Maaf, aku gak bisa bantu soal '{kata}' ya. Itu termasuk konten sensitif/berbahaya.\n\nKalau kamu lagi ada masalah, coba ngobrol sama orang dewasa yang kamu percaya. Aku bisa bantu topik lain yang positif kok!", "ngobrol")]
    
    tingkat = deteksi_tingkat(prompt)
    if tingkat == "image":
        img, err = generate_gambar(prompt)
        return [("image", img, tingkat)] if img else [("text", f"Gagal membuat gambar: {err}", "ngobrol")]
    if tingkat == "remix":
        img, err = remix_gambar_hasil_generate(prompt)
        return [("image", img, "remix")] if img else [("text", f"Gagal remix: {err}", "ngobrol")]
    ...
    for try_model in models:
        try:
            ...
            if full_text:
                result = [("text", full_text, tingkat, try_model)]
                break
        except Exception as e:
            err = str(e)
            if "401" in err:
                toast("API Key salah/expired", "❌")
            elif "429" in err:
                toast("Limit abis, coba model lain...", "⚠️")
            elif "quota" in err.lower():
                toast("Quota abis", "⚠️")
            if try_model == models[-1]:
                result = [("text", f"Error: {err[:80]}. Cek API Key di Secrets.", "ngobrol")]
...
if MAX_CHAT - st.session_state.chat_count == 3:
    st.toast("Sesi ngobrol hampir habis", icon="⚠️")
    ...
    if st.session_state.audio_processed_id != current_audio_id:
        ...
        if voice_text:
            if st.session_state.chat_count >= MAX_CHAT:
                st.error("Sesi ngobrol hari ini sudah habis")
                st.stop()
